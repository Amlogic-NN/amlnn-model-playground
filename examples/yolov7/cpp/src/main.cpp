...
int main(int argc, char **argv)
{
    ...
    std::cout << "YOLOv7 Object Detection Demo" << std::endl;

    fs::create_directory("yolov7_result");
    ...
    // Query IO numbers to ensure we have exactly 3 outputs (the 3 Conv outputs)
    ...
    for (auto &it : fs::directory_iterator(argv[2]))
    {
        ...
        if (outData.empty() || outData.size() < 3)
        {
            std::cerr << "Insufficient outputs returned from network" << std::endl;
            return -1;
        }
        ...
        std::vector<Detection> detections = postprocess(
            ...
            (float *)outData[1].buf, out_shapes[1],
            (float *)outData[2].buf, out_shapes[2],
        ...
        std::string out_path = "yolov7_result/" + it.path().filename().string();
