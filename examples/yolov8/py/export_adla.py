...
def main():
    ...
    amlnn.load_onnx(
        model=args.onnx,
        outputs=[
        "/model.22/Concat_2_output_0",  # <-- Stride 32 (1x144x13x13 grid)
        "/model.22/Concat_1_output_0",  # <-- Stride 16 (1x144x2x26 grid)
        "/model.22/Concat_output_0"     # <-- Stride 8 (1x144x52x52 grid)
    ])
    amlnn.config(
