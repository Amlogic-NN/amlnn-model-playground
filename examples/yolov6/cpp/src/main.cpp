...
int main(int argc, char **argv)
{
    ...
    fs::create_directory("yolov6_result");
    ...
    {
        std::cerr << "Expected 6 YOLOv6 outputs, but model has " << io_num.n_output << " outputs." << std::endl;
    ...
    for (auto &it : fs::directory_iterator(argv[2]))
    {
        ...
        std::string out_path = "yolov6_result/" + it.path().filename().string();
