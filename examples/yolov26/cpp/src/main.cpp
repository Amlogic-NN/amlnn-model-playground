...
int main(int argc, char **argv)
{
    ...
    std::cout << "YOLO26 Demo" << std::endl;
    fs::create_directory("yolo26_result");
    ...
    {
        std::cerr << "Expected 6 YOLO26 outputs, but model has " << io_num.n_output << " outputs." << std::endl;
    ...
    for (auto &it : fs::directory_iterator(argv[2]))
    {
        ...
        std::string out_path = "yolo26_result/" + it.path().filename().string();
