# PaddlePaddle PP-LiteSeg

This example runs PaddlePaddle PP-LiteSeg with AMLNN. The full flow is:
...
5. Check semantic segmentation results.
...
We are distributing the PP-LiteSeg `.onnx` model file in this repository. This model originates from the PaddlePaddle/PaddleSeg project, which is also licensed under the Apache License 2.0. Check and follow the license and usage terms from the model source before your own redistribution or commercial use.
...
### [Download PP-LiteSeg ONNX model here!](https://pub-8378326bd0fe4b1d9312a3847f6316a2.r2.dev/model_zoo/ppseg/pp_liteseg.onnx)

Place the downloaded model under:

```text
examples/ppseg/model/pp_liteseg.onnx
```

## 2. Convert ONNX To ADLA

Run the ADLA export script from `examples/ppseg/py`:

```bash
cd examples/ppseg/py
python export_adla.py \
  --onnx ../model/pp_liteseg.onnx \
...
>
> `activation_dtype` does not apply to `w8a8`. The current PP-LiteSeg model uses `w8a8` and therefore requires a calibration dataset for proper quantization.

After conversion, the generated filename from AMLNN is preserved. For the `pp_liteseg.onnx` model with the current `w8a8` configuration, the expected model path is:

```text
examples/ppseg/model/pp_liteseg_w8a8.adla
```
...
```bash
python ppseg_inference.py \
    --adla ../model/pp_liteseg_w8a8.adla \
    --image-dir ../input
```
...
| -------------- | ------------------------------------------------------------------------------ |
| `--adla`       | Path to the compiled PP-LiteSeg model in `.adla` format.                       |
| `--image-dir`  | Directory containing test images.                                              |
| `--mask-alpha` | (Optional) Controls the transparency (opacity) of the mask. Defaults to `0.5`. |

The script will automatically process all image files (`.jpg`, `.jpeg`, `.png`, `.bmp`) in the specified image directory and save the semantic segmentation visualization results to a `{model_name}_result` folder.
...
* 64-bit: `build/android/arm64-v8a/ppseg_demo`
* 32-bit: `build/android/armeabi-v7a/ppseg_demo`
...
adb push build/android/arm64-v8a/ppseg_demo /data/local/tmp/
adb push ../model/pp_liteseg_w8a8.adla /data/local/tmp/
...
# Usage: ./ppseg_demo <model_path> <image_dir>
./ppseg_demo pp_liteseg_w8a8.adla input/
```

> **Note:** For a 32-bit (`armeabi-v7a`) build, use `build/android/armeabi-v7a/ppseg_demo` and the corresponding 32-bit library path. Replace `pp_liteseg_w8a8.adla` with your actual model file name.
...
* 64-bit: `build/linux/64/ppseg_demo`
* 32-bit: `build/linux/32/ppseg_demo`
...
* 64-bit: `build/yocto/64/ppseg_demo`
* 32-bit: `build/yocto/32/ppseg_demo`
...
adb shell "mkdir -p /data/local/tmp/"
adb push build/linux/64/ppseg_demo /data/local/tmp/
adb push ../model/pp_liteseg_w8a8.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
> **Note:** Replace `pp_liteseg_w8a8.adla` with your actual model file name. Adjust the executable path if using a 32-bit or Yocto build.
...
For each input image, the demo performs semantic segmentation and saves a visualization containing the segmentation mask overlaid on the input image.
...
![PP-LiteSeg segmentation result](./result.jpg)
