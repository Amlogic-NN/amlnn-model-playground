...
| `--conf` | (Optional) Sets the confidence threshold, defaults to 0.4 |
|` --nms`   | (Optional) Sets the IOU threshold, defaults to 0.45 |
...
The executable will be generated at `build/android/yolox_demo` (Note: executable name may vary, verify in build folder).
...
adb push build/android/yolox_demo /data/local/tmp/
adb push ../model/yolox_w8a8.adla /data/local/tmp/
...
./yolox_demo yolox_w8a8.adla input/
```

**Note:** Replace `yolox_w8a8.adla` with your actual model file path.
...
The executable will be generated at `build/linux/64/yolox_demo` (or `build/linux/32/yolox_demo`).
...
The executable will be generated at `build/yocto/64/yolox_demo` (or `build/yocto/32/yolox_demo`).

---

**Run:**

Assuming your target Linux board is accessible via SSH over the network (replace `<device_ip>` with your board's IP address):

```bash
# Push executable and assets to device (adjust build path if using Yocto)
adb shell "mkdir -p /data/local/tmp/" 
adb push build/linux/yolox_demo /data/local/tmp/
adb push ../model/yolox_w8a8.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
**Note:** Replace `yolox_w8a8.adla` with your actual model file name.
...
```
![alt text](./test_image_result.png) 
