...
std::vector<uint8_t> prepare_input_tensor(const cv::Mat &float_img, const amlnn_tensor_attr &attr)
{
    ...
    // Quantize with the model's scale and zero point and saturate to its tensor type.
...
{
    if (out_ptrs.size() != 1 || out_shapes.size() != 1)
    {
        std::cerr << "Expected exactly 1 YOLOX output." << std::endl;
        return {};
    }

    const auto &shape = out_shapes[0];
    if (shape.size() != 2 || shape[0] != 8400 || shape[1] != 85)
    {
        std::cerr << "Unexpected YOLOX output shape; expected [8400, 85]." << std::endl;
        return {};
    }
    ...
    float *data = out_ptrs[0];
    std::vector<Detection> detections;
    int cell_offset = 0;

    // Each row is [tx, ty, tw, th, objectness, 80 class probabilities].
    for (int stride : STRIDES)
    {
        ...
        {
            const float *prediction = data + (cell_offset + cell_idx) * 85;
            float objectness = std::max(0.0f, std::min(1.0f, prediction[4]));
            float best_class_probability = 0.0f;
            int class_id = -1;

            for (int class_idx = 0; class_idx < NUM_CLASSES; ++class_idx)
            {
                float probability = std::max(0.0f, std::min(1.0f, prediction[5 + class_idx]));
            ...
            float score = objectness * best_class_probability;
            if (score <= conf_thresh)
                continue;

            int gx = cell_idx % grid_w;
            int gy = cell_idx / grid_w;
            float center_x = (prediction[0] + gx) * stride;
            float center_y = (prediction[1] + gy) * stride;
            float width = std::exp(std::max(-20.0f, std::min(20.0f, prediction[2]))) * stride;
            float height = std::exp(std::max(-20.0f, std::min(20.0f, prediction[3]))) * stride;

            float x1 = (center_x - width * 0.5f - pad_left) / scale;
            float y1 = (center_y - height * 0.5f - pad_top) / scale;
            float x2 = (center_x + width * 0.5f - pad_left) / scale;
            float y2 = (center_y + height * 0.5f - pad_top) / scale;
            detections.push_back({
                std::max(0.0f, x1), std::max(0.0f, y1),
                std::max(0.0f, x2), std::max(0.0f, y2), score, class_id
            });
        }
    ...
    if (cell_offset != shape[0])
    {
        std::cerr << "Input dimensions generate " << cell_offset
                  << " cells, but the model output has " << shape[0] << "." << std::endl;
        return {};
    }
...
cv::Mat draw_detections(cv::Mat image, const std::vector<Detection> &detections)
{
    ...
    for (const auto &detection : detections)
    {
        ...
        cv::Vec3b color_value = bgr.at<cv::Vec3b>(0, 0);
        cv::Scalar color(color_value[0], color_value[1], color_value[2]);
        ...
        std::string label = COCO_CLASSES[class_id] + ": " + cv::format("%.2f", detection.score);
        int baseline = 0;
        ...
        int brightness = (color_value[0] + color_value[1] + color_value[2]) / 3;
