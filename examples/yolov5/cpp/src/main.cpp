...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "Expected 3 YOLOv5 outputs, but model has " << io_num.n_output << " outputs." << std::endl;
    ...
    for (auto &it : fs::directory_iterator(argv[2]))
    {
        ...
        std::vector<Detection> detections = postprocess(out_ptrs, out_shapes, input_height, input_width, std::make_tuple(preprocessed, scale, pad), SCORE_THRESHOLD, NMS_THRESHOLD);
