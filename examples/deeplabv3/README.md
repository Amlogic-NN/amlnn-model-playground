...
> The PyTorch/Torchvision project (which includes the DeepLabV3 architecture and pre-trained weights) is distributed under the **BSD 3-Clause License**. You can view their full license terms at the official Torchvision repository: [https://github.com/pytorch/vision/blob/main/LICENSE](https://github.com/pytorch/vision/blob/main/LICENSE)

The model pre-processing and post-processing code in this example was partially generated by AI. If any part is similar to existing work and causes concern, please contact us and we will remove or adjust it promptly.## 1. Prepare The ONNX Model

### Download onnx

Download the prepared onnx model and put it under `examples/deeplabv3/model/`:

https://pub-8378326bd0fe4b1d9312a3847f6316a2.r2.dev/model_zoo/deeplabv3/deeplabv3_mobilenet_v3_sim.onnx


Place the downloaded `deeplabv3_mobilenet_v3_sim.onnx` under `examples/deeplabv3/model/`
...
The executable will be generated at `build/android/dlv3_demo` (Note: executable name may vary, verify in build folder).
...
adb push build/android/dlv3_demo /data/local/tmp/
adb push ../model/deeplabv3_w8a8.adla /data/local/tmp/
...
./dlv3_demo deeplabv3_w8a8.adla input/
```

**Note:** Replace `deeplabv3_w8a8.adla` with your actual model file path.
...
The executable will be generated at `build/linux/64/dlv3_demo` (or `build/linux/32/dlv3_demo`).
...
The executable will be generated at `build/yocto/64/dlv3_demo` (or `build/yocto/32/dlv3_demo`).
...
adb shell "mkdir -p /data/local/tmp/" 
adb push build/linux/dlv3_demo /data/local/tmp/
adb push ../model/deeplabv3_w8a8.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
**Note:** Replace `deeplabv3_w8a8.adla` with your actual model file name.
