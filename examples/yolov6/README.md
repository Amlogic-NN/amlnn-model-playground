...
This example runs YOLOv6 with AMLNN. The full flow is:
...
The Yolov6 model is provided by a third party, Check and follow the license and usage terms from the model source before redistribution or commercial use.
...
Run the ADLA export script from `examples/yolov6/py`:

```bash
cd examples/yolov6/py
...
>
> `activation_dtype` does not apply to `w8a8`. The current YOLOv6 model uses `w8a8` and therefore requires a calibration dataset for proper quantization.
...
```text
examples/yolov6/model/model_w8a8.adla
```
...
| `--conf`      | (Optional) Confidence threshold used to filter detections. Defaults to `0.5`.    |
...
* 64-bit: `build/android/arm64-v8a/yolov6_demo`
* 32-bit: `build/android/armeabi-v7a/yolov6_demo`
...
adb push build/android/arm64-v8a/yolov6_demo /data/local/tmp/
...
# Usage: ./yolov6_demo <model_path> <image_dir>
./yolov6_demo model_w8a8.adla input/
```

> **Note:** For a 32-bit (`armeabi-v7a`) build, use `build/android/armeabi-v7a/yolov6_demo` and the corresponding 32-bit library path. Replace `model_w8a8.adla` with your actual model file name.
...
* 64-bit: `build/linux/64/yolov6_demo`
* 32-bit: `build/linux/32/yolov6_demo`
...
* 64-bit: `build/yocto/64/yolov6_demo`
* 32-bit: `build/yocto/32/yolov6_demo`
...
adb shell "mkdir -p /data/local/tmp/"
adb push build/linux/64/yolov6_demo /data/local/tmp/
adb push ../model/model_w8a8.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
![YOLOv6 detection result](./result.png)
