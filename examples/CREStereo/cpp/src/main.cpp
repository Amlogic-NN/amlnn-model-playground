...
int main(int argc, char **argv)
{
    ...
    for (auto &it : fs::directory_iterator(argv[2]))
    {
        ...
        cv::Mat preprocessed;
        float scale = 1.0f;
        std::tuple<int, int> pad = std::make_tuple(0, 0);
        std::vector<uint8_t> prepared_data;

        if (extension == ".bin" || extension == ".qtxt")
        {
            prepared_data = load_direct_input_tensor(it.path().string(), input_attr, input_height, input_width);
            ...
            img = reconstruct_direct_input_image(prepared_data, input_attr, input_height, input_width);
            if (img.empty())
            {
                std::cerr << "Failed to reconstruct direct input image." << std::endl;
                continue;
            }
        }
        else
        {
            img = load_image(it.path().string(), input_height, input_width);
            if (img.empty())
                continue;

            std::tie(preprocessed, scale, pad) = preprocess(img, std::make_tuple(input_height, input_width));
        ...
        std::string out_path = "yolov8_result/" + it.path().stem().string() + "_result.jpg";
