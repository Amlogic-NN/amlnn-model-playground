...
This example runs YAMNet audio classification with AMLNN. The full flow is:
...
5. Check audio classification results.
...
├── input/             # Input WAV audio files and class labels
...
### [Download YAMNet ONNX model here!](https://pub-8378326bd0fe4b1d9312a3847f6316a2.r2.dev/model_zoo/yamnet/yamnet.onnx)
...
After conversion, the generated filename from AMLNN is preserved. For the current `w8a16` with `activation_dtype="f16"` configuration, the expected model path is:
...
* Python 3.10
* Required packages: `numpy`, `amlnn`, `librosa`
...
| ------------- | --------------------------------------------- |
| `--adla`      | Path to the YAMNet `.adla` model.             |
| `--labels`    | Path to the YAMNet class map `.csv` file.     |
| `--audio-dir` | Directory containing `.wav` test audio files. |

The script processes the `.wav` audio files in the specified directory and prints the Top-5 audio classification results for each file.
...
* 64-bit: `build/android/arm64-v8a/yamnet_demo`
* 32-bit: `build/android/armeabi-v7a/yamnet_demo`
...
adb push build/android/arm64-v8a/yamnet_demo /data/local/tmp/
...
# Usage: ./yamnet_demo <model_path> <audio_dir> <labels.csv>
./yamnet_demo yamnet_w8a16.adla input/ input/yamnet_class_map.csv
```

> **Note:** For a 32-bit (`armeabi-v7a`) build, use `build/android/armeabi-v7a/yamnet_demo` and the corresponding 32-bit library path. Replace `yamnet_w8a16.adla` with your actual model filename.
...
* 64-bit: `build/linux/64/yamnet_demo`
* 32-bit: `build/linux/32/yamnet_demo`
...
* 64-bit: `build/yocto/64/yamnet_demo`
* 32-bit: `build/yocto/32/yamnet_demo`
...
adb shell "mkdir -p /data/local/tmp/"
adb push build/linux/64/yamnet_demo /data/local/tmp/
adb push ../model/yamnet_w8a16.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
> **Note:** Replace `yamnet_w8a16.adla` with your actual model filename. Adjust the executable path if using a 32-bit or Yocto build.

## 5. Results

### Performance Feedback

By setting the log level to `INFO`, the program provides real-time performance metrics upon completion. The console log will display essential hardware and execution details, including:
...
For each audio file, the program prints the Top-5 classification results with their respective scores:
