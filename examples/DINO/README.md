...
Download the prepared onnx model and put it under `examples/dino/model/`:
...
Place the downloaded onnx files under `examples/dino/model/`

## 2. Model Conversion
The model conversion is done using the export_adla.py script.
```bash
cd py
Usage:   python export_adla.py --backbone-onnx ../model/dino_backbone.onnx \
...
| `--classifier-onnx` | DINO linear classifier `.onnx` model path                                                                                                                                                                                               |
...
python dino_inference.py \
    --backbone-model ../model/dino_backbone.adla \
    --classifier-model ../model/dino_linear.adla \
    --image-dir ../input \
...
| -------------------- | ---------------------------------------------------------------------------- |
| `--backbone-model`   | Path to the DINO backbone `.adla` model                                      |
| `--classifier-model` | Path to the DINO linear classifier `.adla` model                             |
...
The executable will be generated at `build/android/dino_demo` (Note: executable name may vary, verify in build folder).
...
adb push build/android/dino_demo /data/local/tmp/
adb push ../model/dino_backbone.adla /data/local/tmp/
adb push ../model/dino_linear.adla /data/local/tmp/
...
./dino_demo dino_backbone.adla dino_linear.adla input/ input/labels.txt 10
```
...
The executable will be generated at `build/linux/64/dino_demo` (or `build/linux/32/dino_demo`).
...
The executable will be generated at `build/yocto/64/dino_demo` (or `build/yocto/32/dino_demo`).
