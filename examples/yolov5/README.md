...
This example runs YOLOv5 with AMLNN. The full flow is:
...
The Yolov5 model is provided by a third party, Check and follow the license and usage terms from the model source before redistribution or commercial use.
...
Run the ADLA export script from `examples/yolov5/py`:
...
>
> `activation_dtype` does not apply to `w8a8`. The current YOLOv5 model uses `w8a8` and therefore requires a calibration dataset for proper quantization.
...
* 64-bit: `build/android/arm64-v8a/yolov5_demo`
* 32-bit: `build/android/armeabi-v7a/yolov5_demo`
...
> **Note:** For a 32-bit (`armeabi-v7a`) build, use `build/android/armeabi-v7a/yolov5_demo` and the corresponding 32-bit library path. Replace `model_w8a8.adla` with your actual model file name.
...
* 64-bit: `build/linux/64/yolov5_demo`
* 32-bit: `build/linux/32/yolov5_demo`
...
* 64-bit: `build/yocto/64/yolov5_demo`
* 32-bit: `build/yocto/32/yolov5_demo`
...
![YOLOv5 detection result](./result.jpg)
