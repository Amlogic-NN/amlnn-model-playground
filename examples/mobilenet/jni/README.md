# MobileNet Android JNI Demo

This example runs MobileNet image classification on Amlogic NPU via **AMLNN SDK2** and a JNI bridge. The app is built with Kotlin + Jetpack Compose and loads the AMLNN runtime through `libnnsdk.so`.
...
```text
examples/mobilenet/jni/
├── app/
│   ├── src/main/
│   │   ├── assets/demo/          # Model, labels, and test images (see below)
...
| File | Description |
| ---- | ----------- |
| `mobilenet_v2_1.0_224_quant.adla` | Quantized ADLA model |
| `labels.txt` | ImageNet 1000-class labels |
| `*.jpg` / `*.png` | Optional test images |

To obtain the ADLA model, follow the conversion steps in the parent [MobileNet README](../README.md) (Section 1–2). You can also download a pre-built model from the [Amlogic model zoo](https://huggingface.co/Amlogic-NN/amlnn-adla-models/tree/main).

Copy `labels.txt` from `examples/mobilenet/input/labels.txt`.
...
adb shell am start -n com.example.mobilenet_jni/com.example.NN_JNI.MainActivity
```

On first launch the app copies assets from `assets/demo/` to the external files directory, then runs inference on the selected image. The UI shows Top-5 classification results and timing breakdown.
...
The following are build artifacts or local configuration and are already listed in `.gitignore`:

- `app/build/`, `build/`, `.gradle/`, `app/.cxx/`
- `local.properties`
- `.idea/workspace.xml`, `.idea/caches/`
