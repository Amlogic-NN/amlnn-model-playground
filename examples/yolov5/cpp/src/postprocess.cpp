...
static std::vector<Detection> nms_by_class(const std::vector<Detection> &detections, float iou_threshold)
{
    ...
    {
        std::sort(cls_dets.begin(), cls_dets.end(), [](const Detection &a, const Detection &b) { return a.score > b.score; });
                                   ...
                                   float conf_thresh, float iou_threshold)
{
    ...
    for (int s = 0; s < 3; ++s)
    {
        ...
        int expected_height = input_h / stride;
        int expected_width = input_w / stride;
        if (height != expected_height || width != expected_width)
        {
            std::cerr << "Output " << s << " does not match stride " << stride << ". Got " << height << "x" << width << ", expected " << expected_height << "x" << expected_width << std::endl;
            continue;
        }
        ...
        {
            std::cerr << "Invalid YOLOv5 channel count: " << channels << std::endl;
            continue;
        }
        ...
        for (int y = 0; y < height; ++y)
        {
            for (int x = 0; x < width; ++x)
            {
                ...
                {
                    const float *anchor_data = cell_data + a * values_per_anchor;
                    ...
                    float box_w = tw * 2.0f;
                    float box_h = th * 2.0f;
                    box_w = box_w * box_w * ANCHORS[s][a][0];
                    box_h = box_h * box_h * ANCHORS[s][a][1];
