# Paddlepaddle LiteSeg

This example runs Paddlepaddle LiteSeg with AMLNN. The full flow is:
...
Download the prepared onnx model and put it under `examples/ppseg/model/`:

https://pub-8378326bd0fe4b1d9312a3847f6316a2.r2.dev/model_zoo/ppseg/pp_liteseg.onnx


Place the downloaded `pp_liteseg.onnx` under `examples/ppseg/model/`

## 2. Model Conversion
The model conversion is done using the export_adla.py script.
```
cd py
Usage:   python export_adla.py   --onnx ../model/pp_liteseg.onnx \
...
python ppseg_inference.py \
    --model-path ../model/pp_liteseg_w8a8.adla \
    --image-dir ../input
```
...
The executable will be generated at `build/android/ppseg_demo` (Note: executable name may vary, verify in build folder).
...
adb push build/android/ppseg_demo /data/local/tmp/
...
**Note:** Replace `pp_liteseg_w8a8.adla` with your actual model file path.
...
The executable will be generated at `build/linux/64/ppseg_demo` (or `build/linux/32/ppseg_demo`).
...
The executable will be generated at `build/yocto/64/ppseg_demo` (or `build/yocto/32/ppseg_demo`).
...
adb shell "mkdir -p /data/local/tmp/" 
adb push build/linux/ppseg_demo /data/local/tmp/
adb push ../model/pp_liteseg_w8a8.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
**Note:** Replace `pp_liteseg_w8a8.adla` with your actual model file name.
...
```bash
adb pull model_result/test_image_result.jpg
```
![alt text](./result.jpg) 