...
This example runs YOLOE with AMLNN. The full flow is:
...
The Yoloe-v8s model is provided by a third party and the model weights are not distributed in this repository. Please check and follow the license and usage terms from the model source before your own redistribution or commercial use. We do, however, distribute the accompanying label file required for inference; please refer to the original source dataset for any specific usage terms regarding the labels.
...
### [Download `labels.txt` here!](https://pub-8378326bd0fe4b1d9312a3847f6316a2.r2.dev/model_zoo/yoloe/labels.txt)

Place the downloaded label file under:
...
Run the ADLA export script from `examples/yoloe/py`:
...
>
> `activation_dtype` does not apply to `w8a8`. The current YOLOE model uses `w8a8` and therefore requires a calibration dataset for proper quantization.

After conversion, the generated filename from AMLNN is preserved. For `model.onnx` with the current `w8a8` configuration, the expected model path is:
...
| ------------- | ------------------------------------------------------------------- |
| `--adla`      | Path to the YOLOE `.adla` model.                                    |
| `--image-dir` | Directory containing test images.                                   |
| `--labels`    | (Optional) Path to `labels.txt`, defaults to `../input/labels.txt`. |
| `--conf`      | (Optional) Confidence threshold, defaults to `0.7`.                 |
| `--nms`       | (Optional) IoU threshold used for NMS, defaults to `0.05`.          |

The script automatically processes all image files (`.jpg`, `.jpeg`, `.png`, `.bmp`) in the specified image directory and saves the detection results to a `{model_name}_result` folder.
...
* 64-bit: `build/android/arm64-v8a/yoloe_demo`
* 32-bit: `build/android/armeabi-v7a/yoloe_demo`
...
> **Note:** For a 32-bit (`armeabi-v7a`) build, use `build/android/armeabi-v7a/yoloe_demo` and the corresponding 32-bit library path. Replace `model_w8a8.adla` with your actual model filename.
...
* 64-bit: `build/linux/64/yoloe_demo`
* 32-bit: `build/linux/32/yoloe_demo`
...
* 64-bit: `build/yocto/64/yoloe_demo`
* 32-bit: `build/yocto/32/yoloe_demo`
...
> **Note:** Replace `model_w8a8.adla` with your actual model filename. Adjust the executable path if using a 32-bit or Yocto build.
...
![YOLOE detection result](./result.jpg)
