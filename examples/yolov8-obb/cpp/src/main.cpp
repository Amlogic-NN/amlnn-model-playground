...
const int EXPECTED_OUTPUT_NUM = 9;
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "Expected " << EXPECTED_OUTPUT_NUM << " YOLOv8-OBB outputs, but model has "
    ...
    for (auto &it : fs::directory_iterator(argv[2]))
    {
        ...
        std::string out_path = "yolov8_obb_result/" + it.path().filename().string();
