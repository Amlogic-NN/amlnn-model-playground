...
const int MODEL_INPUT_WIDTH = 1024;
const int MODEL_INPUT_HEIGHT = 1024;
const float SCORE_THRESHOLD = 0.60f;
...
int main(int argc, char **argv)
{
    ...
    std::cout << "YOLOv8 OBB Demo" << std::endl;

    fs::create_directory("yolov8_obb_result");
    ...
    // Query IO numbers to ensure we have exactly 3 output
    amlnn_input_output_num io_num;
    amlnn_query(context, AMLNN_QUERY_IN_OUT_NUM, &io_num, sizeof(io_num));
    if (io_num.n_output != 3)
    {
        std::cerr << "Warning: Expected 3 outputs (boxes, scores), but model has "
    ...
    for (auto &it : fs::directory_iterator(argv[2]))
    {
        ...
        std::vector<Detection> detections = postprocess(
            (float *)outData[0].buf, out_shapes[0], // BBoxes
            (float *)outData[1].buf, out_shapes[1], // Scores
            (float *)outData[2].buf, out_shapes[2], // Mask Coeffs
