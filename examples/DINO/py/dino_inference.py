...
def main():
    parser = argparse.ArgumentParser(description="DINO ADLA Demo")
    parser.add_argument("--backbone-model", required=True, help="Path to DINO backbone .adla model")
    parser.add_argument("--classifier-model", required=True, help="Path to DINO classifier .adla model")
    ...
    print(f"Backbone input shape: {backbone_input_attr['dims']}")
    for i, output in enumerate(backbone_info["outputs"]):
        print(f"Backbone output {i}: name={output['name']}, shape={output['dims']}")
    print(f"Classifier input shape: {classifier_input_attr['dims']}")
    print(f"Classifier output shape: {classifier_info['outputs'][0]['dims']}")
    print()
    ...
    backbone.perf_visualize()
    classifier.perf_visualize()
    backbone.uninit()
    classifier.uninit()
