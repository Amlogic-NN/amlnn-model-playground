                                   ...
                                   float conf_thresh, float iou_threshold)
{
    ...
    auto process_branch = [&](float* data, const std::vector<int>& shape) {
        ...
        // Dynamically deduce grid size from total elements assuming 255 channels (3 anchors * 85)
        ...
        for (int y = 0; y < grid_size; ++y) {
            for (int x = 0; x < grid_size; ++x) {
                for (int a = 0; a < 3; ++a) {
                    // Standard NHWC memory layout index: [Grid_H, Grid_W, 255]
                    int base_idx = y * (grid_size * 255) + x * 255 + a * 85;

                    // Manual Sigmoid to convert raw logits to Objectness Score
                    float obj_conf = sigmoid(data[base_idx + 4]);
                    if (obj_conf < conf_thresh) continue;

                    // Manual Sigmoid on Classes
