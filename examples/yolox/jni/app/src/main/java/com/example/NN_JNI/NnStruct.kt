...
data class TensorAttr(
    ...
    fun effectiveType(): Int {
        val bytesPerElem = if (nElems > 0) size / nElems else 1
        return when {
            bytesPerElem >= 4 -> NnSdk2.TENSOR_FLOAT32
            type == NnSdk2.TENSOR_UINT8 -> NnSdk2.TENSOR_UINT8
            else -> NnSdk2.TENSOR_INT8
        }
    }

    fun byteSize(fallback: Int = 0): Int = when {
        size > 0 -> size
        nElems > 0 -> nElems
        else -> fallback
    }
