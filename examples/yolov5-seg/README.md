...
This example runs YOLOv5 Segmentation with AMLNN. The full flow is:
...
4. Check segmentation images/results.
...
The Yolov5-seg model is provided by a third party, Check and follow the license and usage terms from the model source before redistribution or commercial use.
...
Run the ADLA export script from `examples/yolov5-seg/py`:

```bash
cd examples/yolov5-seg/py
...
>
> `activation_dtype` does not apply to `w8a8`. The current YOLOv5 Segmentation model uses `w8a8` and therefore requires a calibration dataset for proper quantization.
...
```text
examples/yolov5-seg/model/model_w8a8.adla
```
...
```bash
python yolov5_seg_inference.py \
...
| `--conf`       | (Optional) Confidence threshold used to filter detections. Defaults to `0.3`.               |
| `--nms`        | (Optional) IoU threshold used during Non-Maximum Suppression. Defaults to `0.4`.            |
| `--mask-alpha` | (Optional) Controls the transparency (opacity) of the segmentation mask. Defaults to `0.5`. |
...
* 64-bit: `build/android/arm64-v8a/yolov5_seg_demo`
* 32-bit: `build/android/armeabi-v7a/yolov5_seg_demo`
...
adb push build/android/arm64-v8a/yolov5_seg_demo /data/local/tmp/
...
adb shell
cd /data/local/tmp
chmod +x yolov5_seg_demo
export LD_LIBRARY_PATH=/vendor/lib64

# Usage: ./yolov5_seg_demo <model_path> <image_dir>
./yolov5_seg_demo model_w8a8.adla input/
```

> **Note:** For a 32-bit (`armeabi-v7a`) build, use `build/android/armeabi-v7a/yolov5_seg_demo` and the corresponding 32-bit library path. Replace `model_w8a8.adla` with your actual model file name.
...
* 64-bit: `build/linux/64/yolov5_seg_demo`
* 32-bit: `build/linux/32/yolov5_seg_demo`
...
* 64-bit: `build/yocto/64/yolov5_seg_demo`
* 32-bit: `build/yocto/32/yolov5_seg_demo`
...
adb shell "mkdir -p /data/local/tmp/"
adb push build/linux/64/yolov5_seg_demo /data/local/tmp/
adb push ../model/model_w8a8.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
For each input image, the demo performs instance segmentation and saves a visualization containing the detected objects, bounding boxes, and segmentation masks.
...
![YOLOv5 Segmentation result](./result.jpg)
