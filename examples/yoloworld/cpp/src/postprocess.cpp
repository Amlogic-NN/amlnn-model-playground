...
)
{
    ...
    // Combined NHWC heads contain DFL logits followed by prompted class logits.
    for (int output_idx = 0; output_idx < 3; ++output_idx)
    {
        ...
        {
            const float *cell_data = data + cell_idx * channels;
            const float *class_data = cell_data + dfl_channels;
            float max_raw_score = -1e9f;
            int class_id = -1;

            for (int class_idx = 0; class_idx < NUM_CLASSES; ++class_idx)
            {
                float value = class_data[class_idx];
            ...
            for (int side = 0; side < 4; ++side)
            {
                ...
                for (int bin = 0; bin < reg_max; ++bin)
                    max_dfl = std::max(max_dfl, cell_data[channel_offset + bin]);

                float sum_exp = 0.0f;
                float weighted_sum = 0.0f;
                for (int bin = 0; bin < reg_max; ++bin)
                {
                    float exp_value = std::exp(cell_data[channel_offset + bin] - max_dfl);
                    sum_exp += exp_value;
            ...
            int grid_y = cell_idx / width;
            int grid_x = cell_idx % width;
...
)
{
    ...
    for (const auto &detection : detections)
    {
        ...
        if (class_id < 0 || class_id >= static_cast<int>(std::size(WORLD_CLASSES)))
            continue;
        ...
        std::string label = WORLD_CLASSES[class_id] + ": " + cv::format("%.2f", detection.score);
        int baseline = 0;
