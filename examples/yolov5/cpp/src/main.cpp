...
int main(int argc, char **argv)
{
    ...
    std::cout << "YOLOv5 Object Detection Demo" << std::endl;

    fs::create_directory("yolov5_result");
    ...
    // Query IO numbers to ensure we have exactly 3 outputs
    ...
    if (io_num.n_output != 3)
    {
        ...
        // Proceed anyway, depending on the exact model export
    }
    ...
    for (auto &it : fs::directory_iterator(argv[2]))
    {
        ...
        // 5. Postprocess using the 3 float arrays dynamically populated by model_loader
        ...
        std::string out_path = "yolov5_result/" + it.path().filename().string();
