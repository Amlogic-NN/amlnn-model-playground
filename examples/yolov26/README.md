...
This example runs YOLOv26 with AMLNN. The full flow is:
...
└── result.png         # Example detection output
```
...
The yolov26 model is provided by a third party, Check and follow the license and usage terms from the model source before redistribution or commercial use.
...
Run the ADLA export script from `examples/yolov26/py`:

```bash
cd examples/yolov26/py
...
>
> `activation_dtype` does not apply to `w8a8`. The current YOLOv26 model uses `w8a8` and therefore requires a calibration dataset for proper quantization.
...
```text
examples/yolov26/model/model_w8a8.adla
```
...
* 64-bit: `build/android/arm64-v8a/yolov26_demo`
* 32-bit: `build/android/armeabi-v7a/yolov26_demo`
...
adb push build/android/arm64-v8a/yolov26_demo /data/local/tmp/
...
adb shell
cd /data/local/tmp
chmod +x yolov26_demo
export LD_LIBRARY_PATH=/vendor/lib64

# Usage: ./yolov26_demo <model_path> <image_dir>
./yolov26_demo model_w8a8.adla input/
```

> **Note:** For a 32-bit (`armeabi-v7a`) build, use `build/android/armeabi-v7a/yolov26_demo` and the corresponding 32-bit library path. Replace `model_w8a8.adla` with your actual model file name.
...
* 64-bit: `build/linux/64/yolov26_demo`
* 32-bit: `build/linux/32/yolov26_demo`
...
* 64-bit: `build/yocto/64/yolov26_demo`
* 32-bit: `build/yocto/32/yolov26_demo`
...
adb shell "mkdir -p /data/local/tmp/"
adb push build/linux/64/yolov26_demo /data/local/tmp/
adb push ../model/model_w8a8.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
![YOLOv26 detection result](./result.png)
