...
int main(int argc, char **argv)
{
    ...
    fs::create_directory("yolox_result");

    // Initialize the network and cache the tensor metadata once.
    ...
    {
        std::cerr << "Expected 1 YOLOX output, but model has "
    ...
    for (auto &it : fs::directory_iterator(argv[2]))
    {
        ...
        std::string out_path = "yolox_result/" + it.path().filename().string();
