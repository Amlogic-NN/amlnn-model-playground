...
This example runs YOLOv8-OBB with AMLNN. The full flow is:
...
The Yolov8n-obb model is provided by a third party, Check and follow the license and usage terms from the model source before redistribution or commercial use.
...
Run the ADLA export script from `examples/yolov8-obb/py`:

```bash
cd examples/yolov8-obb/py
python export_adla.py \
  --onnx ../model/model.onnx \
  --dataset-path ../../../resource/dota_dataset.txt \
  --target-platform 007 \
  --output-dir ../model
```
...
| `--dataset-path`    | (Optional) Path to a `.txt` file containing the input image paths used for quantization calibration. Required for `w8a8` and `w8a16 (i16)` configuration for proper quantization.[1] [2] |
...
>
> `activation_dtype` does not apply to `w8a8`. The current YOLOv8-OBB model uses `w8a8` and therefore requires a calibration dataset for proper quantization.

> **[2] Quantization dataset selection**
>
> Refer to the [resource README](../../resource/README.md) for more information.
...
```text
examples/yolov8-obb/model/model_w8a8.adla
```
...
```bash
python yolov8_obb_inference.py \
...
* 64-bit: `build/android/arm64-v8a/yolov8_obb_demo`
* 32-bit: `build/android/armeabi-v7a/yolov8_obb_demo`
...
adb push build/android/arm64-v8a/yolov8_obb_demo /data/local/tmp/
...
adb shell
cd /data/local/tmp
chmod +x yolov8_obb_demo
export LD_LIBRARY_PATH=/vendor/lib64

# Usage: ./yolov8_obb_demo <model_path> <image_dir>
./yolov8_obb_demo model_w8a8.adla input/
```

> **Note:** For a 32-bit (`armeabi-v7a`) build, use `build/android/armeabi-v7a/yolov8_obb_demo` and the corresponding 32-bit library path. Replace `model_w8a8.adla` with your actual model file name.
...
* 64-bit: `build/linux/64/yolov8_obb_demo`
* 32-bit: `build/linux/32/yolov8_obb_demo`
...
* 64-bit: `build/yocto/64/yolov8_obb_demo`
* 32-bit: `build/yocto/32/yolov8_obb_demo`
...
adb shell "mkdir -p /data/local/tmp/"
adb push build/linux/64/yolov8_obb_demo /data/local/tmp/
adb push ../model/model_w8a8.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
For each input image, the demo performs oriented object detection and saves a visualization containing the detected oriented bounding boxes.
...
![YOLOv8-OBB detection result](./result.jpg)
