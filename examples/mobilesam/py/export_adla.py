...
def main():
    parser = argparse.ArgumentParser(description="Export MobileSAM ONNX models to ADLA")
    parser.add_argument("--enc-onnx", required=True, help="Path to encoder ONNX model")
    parser.add_argument("--det-onnx", required=True, help="Path to detector ONNX model")
    ...
    enc_model_path = Path(args.enc_onnx).resolve()
    det_model_path = Path(args.det_onnx).resolve()
    output_dir = Path(args.output_dir).resolve()

    if not enc_model_path.is_file():
        raise FileNotFoundError(f"Model not found: {enc_model_path}")
    ...
    # Export encoder
    search_dirs = {Path.cwd().resolve(), enc_model_path.parent}
    known_adla_files = snapshot_adla_files(search_dirs)

    amlnn = AMLNN()

    amlnn.load_onnx(model=str(enc_model_path))
    ...
    updated_adla_files = find_updated_adla_files(search_dirs, known_adla_files)
    if not updated_adla_files:
        raise RuntimeError("Detector export_adla did not create or update a .adla file")
