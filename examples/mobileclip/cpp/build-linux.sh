...
usage() {
    ...
    echo "  -a <target>    : Target arch for linux mode: aarch64 or armhf (default: aarch64)"
    echo "  -b <arch_bits> : Arch bits: 32 or 64 for linux/yocto (default: 64)"
...
if [[ "${TARGET_ARCH}" == "armhf" || "${TARGET_ARCH}" == "arm" ]]; then
    ARCH_BITS=32
    TARGET_ARCH=armhf
fi
