...
parent_dir = os.path.abspath(os.path.join(current_dir, '..'))  # 获取上一级目录
sys.path.append(parent_dir)  # 将上一级目录添加到 sys.path
...
def modified_forward(
    ...
    """
    device = next(self.embed.parameters()).device  # 获取 embed 权重所在设备
    x = x.to(device)
    language = language.to(device)
    text_norm = text_norm.to(device)

    x_length = torch.full((1,), x.shape[1], dtype=torch.int32, device=device)  # 1-D 张量
