...
This example runs YOLOWorld with AMLNN. The full flow is:
...
The Yoloworld model is provided by a third party, Check and follow the license and usage terms from the model source before redistribution or commercial use.
...
Run the ADLA export script from `examples/yoloworld/py`:

```bash
cd examples/yoloworld/py
...
>
> `activation_dtype` does not apply to `w8a8`. The current YOLOWorld model uses `w8a8` and therefore requires a calibration dataset for proper quantization.
...
```text
examples/yoloworld/model/model_w8a8.adla
```
...
* 64-bit: `build/android/arm64-v8a/yoloworld_demo`
* 32-bit: `build/android/armeabi-v7a/yoloworld_demo`
...
adb push build/android/arm64-v8a/yoloworld_demo /data/local/tmp/
...
adb shell
cd /data/local/tmp
chmod +x yoloworld_demo
export LD_LIBRARY_PATH=/vendor/lib64

# Usage: ./yoloworld_demo <model_path> <image_dir>
./yoloworld_demo model_w8a8.adla input/
```

> **Note:** For a 32-bit (`armeabi-v7a`) build, use `build/android/armeabi-v7a/yoloworld_demo` and the corresponding 32-bit library path. Replace `model_w8a8.adla` with your actual model file name.
...
* 64-bit: `build/linux/64/yoloworld_demo`
* 32-bit: `build/linux/32/yoloworld_demo`
...
* 64-bit: `build/yocto/64/yoloworld_demo`
* 32-bit: `build/yocto/32/yoloworld_demo`
...
adb shell "mkdir -p /data/local/tmp/"
adb push build/linux/64/yoloworld_demo /data/local/tmp/
adb push ../model/model_w8a8.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
For each input image, the demo performs object detection using the predefined classes and saves a visualization containing the detected bounding boxes and class labels.

The result images are saved inside the model result directory using the input image name. For example:

```text
<model_name>_result/
└── test_image_result.png
```
...
![YOLOWorld detection result](./result.jpg)
