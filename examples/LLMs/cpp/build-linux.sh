...
usage() {
    ...
    echo "  -m <mode>      : Build mode: 'linux' or 'yocto' (default: yocto)"
...
if [[ "${BUILD_MODE}" == "yocto" ]]; then
    ...
    if [[ -x "${STRIP_TOOL}" ]]; then
        "${STRIP_TOOL}" --strip-unneeded "${BUILD_DIR}/LLM_demo"
    else
...
# We do not support llmsdk for buildroot yet
...
# # 2. Allow override via GCC_COMPILER environment variable
# GCC_COMPILER=${GCC_COMPILER:-${DEFAULT_COMPILER}}

# # 3. Set compilers and strip tool
...
# # 4. Validate compiler is in PATH
