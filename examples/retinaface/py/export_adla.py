...
def main():
    parser = argparse.ArgumentParser(description="Export RetinaFace ONNX to ADLA")
    parser.add_argument("--onnx", required=True, help="Path to RetinaFace ONNX model")
    ...
    parser.add_argument("--adla", help="Optional output .adla path")
    ...
    amlnn = AMLNN()
    amlnn.load_onnx(model=args.onnx)
    amlnn.config(
