...
This example runs YOLOX with AMLNN. The full flow is:
...
The Yolox model is provided by a third party, Check and follow the license and usage terms from the model source before redistribution or commercial use.
...
Run the ADLA export script from `examples/yolox/py`:

```bash
cd examples/yolox/py
...
>
> `activation_dtype` does not apply to `w8a8`. The current YOLOX model uses `w8a8` and therefore requires a calibration dataset for proper quantization.
...
```text
examples/yolox/model/model_w8a8.adla
```
...
| `--conf`      | (Optional) Confidence threshold used to filter detections. Defaults to `0.4`.     |
...
* 64-bit: `build/android/arm64-v8a/yolox_demo`
* 32-bit: `build/android/armeabi-v7a/yolox_demo`
...
adb push build/android/arm64-v8a/yolox_demo /data/local/tmp/
...
# Usage: ./yolox_demo <model_path> <image_dir>
./yolox_demo model_w8a8.adla input/
```

> **Note:** For a 32-bit (`armeabi-v7a`) build, use `build/android/armeabi-v7a/yolox_demo` and the corresponding 32-bit library path. Replace `model_w8a8.adla` with your actual model file name.
...
* 64-bit: `build/linux/64/yolox_demo`
* 32-bit: `build/linux/32/yolox_demo`
...
* 64-bit: `build/yocto/64/yolox_demo`
* 32-bit: `build/yocto/32/yolox_demo`
...
adb shell "mkdir -p /data/local/tmp/"
adb push build/linux/64/yolox_demo /data/local/tmp/
adb push ../model/model_w8a8.adla /data/local/tmp/
adb push ../input/ /data/local/tmp/
...
![YOLOX detection result](./result.png)
